...
describe('onConnect', () => {
  ...
  it('Does not connect when autoConnect is false', () => {
    const provider = new _.default(_mockWs.TEST_WS_URL, 0);
    expect(provider.isConnected).toBe(false);
  });
  it('Does connect when autoConnect is true', async () => {
    const provider = new _.default(_mockWs.TEST_WS_URL, 1);
  ...
  it('Connects to the second endpoint when the first is unreachable', async () => {
    ...
    const endpoints = ['ws://localhost:9956', _mockWs.TEST_WS_URL];
    const provider = new _.default(endpoints, 1);
    await sleepMs(100); // Hack to give the provider time to connect

    expect(provider.isConnected).toBe(true);
  });
  it('Connects to the second endpoint when the first is dropped', async () => {
    const endpoints = [_mockWs.TEST_WS_URL, 'ws://localhost:9957'];
    mocks.push((0, _mockWs.mockWs)([], endpoints[1]));
    const provider = new _.default(endpoints, 1);
    await sleepMs(100); // Hack to give the provider time to connect
    // Check that first server is connected

    expect(mocks[0].server.clients().length).toBe(1);
    expect(mocks[1].server.clients().length).toBe(0); // Close connection from first server

    mocks[0].server.clients()[0].close();
    await sleepMs(100); // Check that second server is connected
  ...
  it('Round-robin of endpoints on WsProvider', async () => {
    const rounds = 5;
    const endpoints = [_mockWs.TEST_WS_URL, 'ws://localhost:9956', 'ws://localhost:9957', 'ws://invalid:9956', 'ws://localhost:9958'];
    mocks.push((0, _mockWs.mockWs)([], endpoints[1]));
    mocks.push((0, _mockWs.mockWs)([], endpoints[2]));
    mocks.push((0, _mockWs.mockWs)([], endpoints[4]));
    const mockNext = [mocks[1], mocks[2], mocks[3], mocks[0]];
    const provider = new _.default(endpoints, 1);
    ...
    for (let round = 0; round < rounds; round++) {
      for (let index = 0; index < mocks.length; index++) {
        ...
        expect(provider.isConnected).toBe(true); // Close connection from first server
